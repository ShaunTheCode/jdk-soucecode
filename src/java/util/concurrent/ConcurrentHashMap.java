...
public class ConcurrentHashMap<K,V> extends AbstractMap<K,V>
     ...
     * put触发扩容时, 传入的nextTab值是null
     * transferIndex 表示转移时的下标，初始为扩容前的 length。
     * 我们假设长度是 32
     *
     * 在执行put操作的线程中，第一个发现需要扩容的线程负责分配新数组、开始转移部分Node，每次处理一个Node.
     *
     * 此后，其他执行put操作的线程, 先在原数组中定位到Node
     *
     * 如果遇到处理过的Node(即FowardingNode)，则参与到转移Node工作
     *
     * 如果遇到正在处理中的Node，则会阻塞在这个Node上, 直到扩容线程完成此Node的数据转移, 然后把Node修改为处理过并释放锁,
     * 此线程继续执行发现Node被修改为FowardingNode, 则同样参与到转移Node工作
     *
     * 如果以上都没有, 则正常put数据
     *
     * 执行get操作的线程不参与转移工作，遇到FordwardingNode则到新数组查询。
     *
     * 可以看到, ConcurentHashMap选择先复制数据, 再更新table变量, 解决HashMap的get null值问题
     *
     * 然后使用synchronized加锁解决复制数据过程中, put新值的问题
     *
     *
     * 最后使用FordwardingNode解决锁粒度大的问题, 使锁的粒度从table数组变小为数组中的一个元素(Node), 同时将转移数据的任务分解, 使得其他put线程, 在碰到处理过的Node时, 不是只阻塞等待, 而是去帮助转移数据, 大大加快了转移数据的速度.
     */
