...
public class HashMap<K,V> extends AbstractMap<K,V>
     ...
     *
     * 多线程的put可能导致元素的丢失
     * 多个线程同时走到#1或#2处时,再往下执行,就会覆盖上一个操作添加的元素, 造成元素丢失
     * @param hash hash for key
     * @param key the key
    ...
    final V putVal(int hash, K key, V value, boolean onlyIfAbsent,
        ...
        if ((tab = table) == null || (n = tab.length) == 0)
            n = (tab = resize()).length;
        // 通过hash值计算key在hash表中的位置，并将这个位置上的元素赋值给p，
        // 如果是空的则new一个新的node放在这个位置上
        if ((p = tab[i = (n - 1) & hash]) == null)
            // #1---------------------------------
            tab[i] = newNode(hash, key, value, null);
        // 发生哈希冲突的几种情况
        else {
            // e 存放和要插入的元素,key值相同的节点， k 存放该当前桶节点的key
            Node<K,V> e; K k;
            //第一种，插入的key-value的hash值，key都与当前节点的相等，e = p，则表示为首节点
            if (p.hash == hash &&
                ((k = p.key) == key || (key != null && key.equals(k))))
                e = p;
            //第二种，hash值不等于首节点或者key不相等，判断该p是否属于红黑树的节点
            else if (p instanceof TreeNode)
                /**为红黑树的节点，则在红黑树中进行添加，如果该节点已经存在，
                 则返回该节点（不为null），如果添加成功返回null
                 **/
        ...
        //到了此步骤，则表明待插入的key-value是没有key的重复，因为插入成功,e节点的值为null
        //修改次数+1
        ++modCount;
     ...
     * 重点代码在于#1和#2两句
     *
     * 在代码#3位置，用新计算的容量new了一个新的hash表，#4将新创建的空hash表赋值给实例变量table。
     *
     * 注意此时实例变量table是空的。
     *
